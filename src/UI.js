...
function buildFleasioUI(state, config, save) {
    ...
    function createStatsToggle(initialMode, onToggleChange, onModeChange) {
        const row = document.createElement("div");
        Object.assign(row.style, {
            display: "flex", justifyContent: "space-between", alignItems: "center",
    ...
    function updateStatsDisplay() {
        const parts = [];
        if (statsMode === "ping" || statsMode === "both") {
            parts.push(`Ping: ${lastPingMs !== null ? lastPingMs + "ms" : "--ms"}`);
        }
        if (statsMode === "fps" || statsMode === "both") {
    ...
    function fpsLoop(now) {
        ...
        if (statsEnabled && (statsMode === "fps" || statsMode === "both")) {
    ...
    quickMenuOverlay.content.appendChild(createStatsToggle(
        ...
        (mode) => {
            statsMode = mode;
            if (statsEnabled) { stopStats(); startStats(); }
        }
    ));
