...
function buildFleasioUI(state, config, save) {
    ...
    // --- Ping GUI — a small draggable HUD label. Measures HTTP round-trip
    // time to the veck.io domain as an approximation of network latency
    // (there's no access to the game's own internal networking, which is
    // almost certainly a WebSocket, not something a userscript can time). ---
    const ping = document.createElement("div");
    ping.id = "fleasio-ping";
    ...
    const restoredPingPos = state.positions.ping;
    if (restoredPingPos) {
        ping.style.left = restoredPingPos.left;
        ping.style.top = restoredPingPos.top;
    } else {
        ping.style.left = "10px";
        ping.style.top = "20px";
    }
    ping.textContent = "Ping: --ms";
    document.documentElement.appendChild(ping);

    let pingDragging = false, pingDragStart = { x: 0, y: 0 }, pingStartPos = { x: 0, y: 0 };
    ping.addEventListener("pointerdown", (e) => {
        if (!state.moveMode || state.uiHidden) return;
        pingDragStart = { x: e.clientX, y: e.clientY };
        const r = ping.getBoundingClientRect();
        pingStartPos = { x: r.left, y: r.top };
        pingDragging = true;
        ping.setPointerCapture(e.pointerId);
    });
    ping.addEventListener("pointermove", (e) => {
        if (!pingDragging) return;
        const dx = e.clientX - pingDragStart.x;
        const dy = e.clientY - pingDragStart.y;
        ping.style.left = (pingStartPos.x + dx) + "px";
        ping.style.top = (pingStartPos.y + dy) + "px";
    });
    ["pointerup", "pointercancel", "pointerleave"].forEach(evt =>
        ping.addEventListener(evt, () => {
            if (pingDragging) {
                state.positions.ping = { left: ping.style.left, top: ping.style.top };
                persistPositions();
            }
            pingDragging = false;
        })
    ...
    quickMenuOverlay.content.appendChild(createToggle("Ping GUI", false, (val) => {
        if (val) {
    ...
    settingsOverlay.content.appendChild(createToggle("Move UI", state.moveMode, (val) => {
        ...
        ping.style.pointerEvents = val ? "auto" : "none";
        ping.style.cursor = val ? "move" : "default";
    }));
    ...
    // Main Menu / Quick Menu / Settings keyboard shortcuts — all rebindable.
    // Never fire while focus is in a text field, since these keys (Right
    // Shift, Right Ctrl by default) are also normal modifier keys people use
    // while typing. Settings has no default binding unless you set one.
